    ...
    try {
      switch (calculationType) {
        case "simpleInterest": {
          ...
          const timeYears = calculateTimeInYears(
            formData.timePeriod,
            formData.timeUnit
          );
          ...
          for (let year = 1; year <= Math.ceil(timeYears); year++) {
            const yearInterest =
              principal * rate * Math.min(1, timeYears - (year - 1));
            breakdown.push({
              year,
          ...
          return {
            ...
            summary: [
              ...
              {
                label: "Simple Interest",
                value: `$${simpleInterest.toLocaleString()}`,
              },
              {
                label: "Total Amount",
        ...
        case "compoundInterest": {
          ...
          const compoundAmount =
            principal * Math.pow(1 + rate / n, n * timeYears);
          ...
          return {
            ...
            summary: [
              ...
              {
                label: "Compound Interest",
                value: `$${compoundInterest.toLocaleString()}`,
              },
              {
                label: "Final Amount",
                value: `$${compoundAmount.toLocaleString()}`,
              },
              ...(deposit > 0
                ? [
                    {
                    ...
                    {
                      label: "Total with Deposits",
                      value: `$${totalWithDeposits.toLocaleString()}`,
                    },
                  ]
                : []),
            ],
        ...
        case "creditCardInterest": {
          ...
          const minPaymentPercent =
            parseFloat(formData.minimumPaymentPercent) / 100;
          ...
          while (minRemaining > 0 && minMonths < 600) {
            const minPayment = Math.max(
              minPaymentAmount,
              minRemaining * minPaymentPercent
            );
          ...
          return {
            ...
            summary: [
              ...
              {
                label: "Total Paid",
                value: `$${(balance + totalInterest).toLocaleString()}`,
              },
              { label: "With Minimum Payment", value: `${minMonths} months` },
              {
                label: "Min Payment Interest",
                value: `$${minTotalInterest.toLocaleString()}`,
              },
            ],
          };
        }
        ...
        case "interestComparison": {
          ...
          for (let year = 1; year <= timeYears; year++) {
            breakdown.push({
              ...
              difference:
                principal * Math.pow(1 + rate2, year) -
                principal * Math.pow(1 + rate1, year),
            });
          }
          ...
          return {
            ...
            summary: [
              { label: "Principal", value: `$${principal.toLocaleString()}` },
              {
                label: `Rate 1 (${(rate1 * 100).toFixed(2)}%)`,
                value: `$${amount1.toLocaleString()}`,
              },
              {
                label: `Rate 2 (${(rate2 * 100).toFixed(2)}%)`,
                value: `$${amount2.toLocaleString()}`,
              },
              {
              ...
              },
              {
                ...
                value: `${((difference / amount1) * 100).toFixed(1)}%`,
              },
            ],
          };
        }
        ...
        case "effectiveRate": {
          ...
          return {
            ...
            summary: [
              {
                label: "Nominal Annual Rate",
                value: `${(nominalRate * 100).toFixed(2)}%`,
              },
              {
                label: "Compounding Frequency",
        ...
        case "loanInterest": {
          ...
          const inflationAdjustedInterest =
            totalInterest / Math.pow(1 + inflationRate, timeYears);
          ...
          return {
            ...
            summary: [
              ...
              {
                label: "Monthly Payment",
                value: `$${monthlyPayment.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
              },
              {
                label: "Total Interest",
                value: `$${totalInterest.toLocaleString()}`,
              ...
              { label: "Real Interest Rate", value: `${realRate.toFixed(2)}%` },
              {
                ...
                value: `$${afterTaxInterest.toLocaleString()}`,
              },
            ],
          };
        }
      ...
      case "compoundInterest":
        return {
          bar: {
            data: {
              labels:
                results.breakdown?.map((item) => `Year ${item.year}`) || [],
              datasets: [
                {
                  label: "Principal",
      ...
      <div className="card bg-gradient-to-br from-primary/5 to-transparent mx-2 md:mx-0">
        <div className="text-center mb-4 md:mb-6">
          ...
          <div className="text-2xl md:text-3xl font-bold text-primary mb-2">
            {results.type === "simpleInterest" &&
              `$${results.totalAmount.toLocaleString()}`}
            {results.type === "compoundInterest" &&
              `$${results.compoundAmount.toLocaleString()}`}
            {results.type === "creditCardInterest" &&
              `${results.monthsToPayoff} months`}
            {results.type === "interestComparison" &&
              `$${results.difference.toLocaleString()}`}
            {results.type === "effectiveRate" &&
              `${results.effectiveRate.toFixed(2)}%`}
            {results.type === "loanInterest" &&
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="px-2 md:px-0">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-2 text-sm md:text-base text-center md:text-left">
          Calculate simple interest, compound interest, credit card interest,
          and compare different rates.
        </p>
      </div>
        ...
        <div className="space-y-4 md:space-y-0 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-6">
          <div>
            ...
            <p className="text-sm text-text/70">
              Interest calculated only on the principal amount. Common for
              short-term loans and some investments.
            </p>
          </div>
          <div>
            <h4 className="font-semibold mb-2 text-base">Compound Interest</h4>
            <p className="text-sm text-text/70">
              Interest calculated on principal plus accumulated interest. The
              most common type for savings and investments.
            </p>
          </div>
          <div>
            <h4 className="font-semibold mb-2 text-base">APR vs EAR</h4>
            <p className="text-sm text-text/70">
              APR is the nominal rate, EAR is the actual rate after compounding.
              EAR is always higher than APR with frequent compounding.
            </p>
          </div>
        </div>
      </div>
