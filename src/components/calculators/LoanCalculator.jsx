    ...
    try {
      ...
      while (balance > 0.01 && paymentNumber <= totalPayments) {
        ...
        if (extraPayment > 0) {
          const shouldApplyExtra = () => {
            if (
              ...
              paymentNumber === extraPaymentStart
            )
              return true;
            if (
              extraPaymentFrequency === "monthly" &&
            ...
            if (
              ...
              paymentNumber >= extraPaymentStart &&
              paymentNumber % (paymentsPerYear / 4) === 0
            )
              return true;
            if (
              extraPaymentFrequency === "annual" &&
              paymentNumber >= extraPaymentStart &&
              paymentNumber % paymentsPerYear === 0
            )
              return true;
            return false;
          };
        ...
        switch (paymentFrequency) {
          ...
          case "semi-monthly":
            paymentDate.setDate(
              paymentDate.getMonth() === paymentDate.getMonth() ? 15 : 1
            );
            paymentDate.setMonth(
              paymentDate.getMonth() + (paymentDate.getDate() === 1 ? 0 : 1)
            );
            break;
          case "monthly":
      ...
      const prepaymentPenaltyAmount =
        prepaymentPenalty > 0
          ? (loanAmount - totalPrincipalPaid) * prepaymentPenalty
          : 0;
      ...
      const apr =
        (Math.pow(totalPayment / loanAmount, 1 / years) - 1) * 100 ||
        interestRate * 100;
      ...
      const interestWithoutExtra = scheduleWithoutExtra.reduce(
        (sum, row) => sum + row.interest,
        0
      );
      const interestSavings = interestWithoutExtra - totalInterestPaid;
      const timeSavingsMonths =
        years * 12 - schedule.length / (paymentsPerYear / 12);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-2">
          Calculate loan payments, total interest, APR, and create detailed
          amortization schedules for personal, auto, student, business, and
          other loans.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="card space-y-6">
          ...
          <h3 className="font-semibold pt-4 border-t border-border">
            Fees & Penalties
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="card bg-gradient-to-br from-primary/5 to-transparent">
                <div className="text-center mb-6">
                  ...
                  <p className="text-sm text-text/60">
                    {formData.loanType.replace(/([A-Z])/g, " $1")} Loan •{" "}
                    {results.years} years
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-3 bg-surface rounded-lg">
                    ...
                    <div className="text-xs text-text/60 mt-1">
                      {(
                        (results.totalInterestPaid / results.loanAmount) *
                        100
                      ).toFixed(1)}
                      % of loan
                    </div>
                  ...
                  {results.totalExtraPayments > 0 && (
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="font-medium text-green-600">
                        ${results.totalExtraPayments.toLocaleString()}
                      </span>
                    </div>
                  )}
                  {results.prepaymentPenaltyAmount > 0 && (
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="font-medium text-orange-600">
                        ${results.prepaymentPenaltyAmount.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    {results.timeSavingsMonths > 0 && (
                      <div className="p-3 border border-green-200 dark:border-green-800 rounded-lg">
                        ...
                        <div className="text-xl font-bold text-green-600">
                          {Math.floor(results.timeSavingsMonths / 12)}y{" "}
                          {Math.round(results.timeSavingsMonths % 12)}m
                        </div>
                      </div>
                    )}
                    {results.taxSavings > 0 && (
                    ...
                    <div className="p-3 border border-border rounded-lg">
                      ...
                      <div className="text-xl font-bold">{results.apr}%</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium capitalize">
                    {formData.loanType.replace(/([A-Z])/g, " $1")}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {(results.interestRate * 100).toFixed(3)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Term</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium capitalize">
                    {formData.compoundFrequency.replace("-", " ")}
                  </span>
                </div>
              </div>
            )}
            ...
            {results && (
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {results.summary.paymentsPerYear}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    $
                    {(
                      results.totalFees + results.prepaymentPenaltyAmount
                    ).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between border-t border-border pt-2">
                  ...
                  <span className="font-bold">
                    ${(results.totalCost - results.taxSavings).toLocaleString()}
                  </span>
                </div>
              </div>
            )}
